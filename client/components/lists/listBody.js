...
BlazeComponent.extendComponent({
  ...
  addCard(evt) {
    ...
    if (title) {
      ...
      // to appear
      const cardCount = this.data().cards(this.idOrNull(swimlaneId)).length;
      ...
      window.cardsintro = introJs();
      window.cardsintro.setOptions({
        ...
        steps: [
          {
            ...
            title: 'פתיחת הכרטיס',
            intro:
              'ניתן ללחוץ על כרטיסיית המשימה בכדי להרחיב אותה ולגשת לפרטים נוספים.',
          },
          {
            element: '.minicard',
            intro:
          ...
          {
            element: '.js-change-board-color',
            title: 'התאימו אישית את הלוח',
            intro: 'אין שני תהליכי עבודה זהים, אז למה לא לתת לכל לוח מראה ותחושה ייחודיים? ביכולתכם להתאים אישית את רקע הלוח שלכם.',
            position: 'right'
          },

          {
            element: '.js-toggle-sidebar',
            title: 'הגדירו את הלוח',
            intro:
              'לחצו על אייקון התפריט על מנת לפתוח את התפריט הצדדי. שם תוכלו להזמין חברים ללוח, לשנות את הגדרות הלוח או לצפות בפעילויות של כרטיסיות המשימה.',
          },
        ],
      });
  ...
  canSeeAddCard() {
    return !this.reachedWipLimit() && Utils.canModifyCard();
  },
...
BlazeComponent.extendComponent({
  onCreated() {
    ...
    arr = [];
    _.forEach(
      ReactiveCache.getBoard(currentBoardId).customFields(),
      function (field) {
...
BlazeComponent.extendComponent({
  onCreated() {
    ...
    if (Utils.boardView() === 'board-view-swimlanes')
      ...
      this.swimlaneId = ReactiveCache.getSwimlane({
  ...
  swimlanes() {
    ...
    if (swimlanes.length) this.selectedSwimlaneId.set(swimlanes[0]._id);
    return swimlanes;
  },
...
BlazeComponent.extendComponent({
  ...
  onCreated() {
    ...
    if (this.isTemplateSearch) {
      const boardId = (ReactiveCache.getCurrentUser().profile || {})
        .templatesBoardId;
      if (boardId) {
    ...
    return [
      {
        ...
        'click .js-minicard'(evt) {
          ...
          if (!this.isTemplateSearch || this.isCardTemplateSearch) {
            ...
            element.sort = ReactiveCache.getSwimlane(
              this.swimlaneId,
            ).lists().length;
            element.type = 'list';
            _id = element.copy(this.boardId, this.swimlaneId);
          } else if (this.isSwimlaneTemplateSearch) {
            element.sort = ReactiveCache.getBoard(
              this.boardId,
            ).swimlanes().length;
...
(class extends Spinner {
  ...
  spinnerInView() {
    ...
    const spinnerViewPosition =
      this.spinner.offsetTop -
      this.container.offsetTop +
      this.spinner.clientHeight;
